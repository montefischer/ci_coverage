...
def main(Nx, Ny, beta, Y_MAX, delta, eps_abs, eps_rel, run_name):
    ...
    np.savez(run_dir/"data", G=G, Xc=Xc, Yc_grid=Yc_grid)
    ...
    plt.tight_layout()
    plt.savefig(run_dir/'pde_solution_centered.png', dpi=150, bbox_inches='tight')
    ...
    plt.tight_layout()
    plt.savefig(run_dir/'pde_residual_centered.png', dpi=150, bbox_inches='tight')
    ...
    plt.tight_layout()
    plt.savefig(run_dir/'pde_profiles_centered.png', dpi=150, bbox_inches='tight')
    ...
    plt.tight_layout()
    plt.savefig(run_dir/"marginals_near_boundary.png")
    plt.show()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--eps_abs", type=float, default=1e-4)
    parser.add_argument("--eps_rel", type=float, default=1e-4)
    ...
    # Compute dependent quantity
    z = z_from_delta(args.delta)

    # Generate identifiers
    run_name = generate_slug(3)   # e.g. "ancient-lava-swan"

    # Create run directory
    root = Path(args.runs_dir)
    run_dir = root / f"{run_name}"
    ...
    main(args.Nx, args.Ny, args.beta, args.Y_MAX, args.delta, args.eps_abs, args.eps_rel, run_name)
